        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ trans('lang.driver_disbursement') }}</li>
                </ol>
            </div>
            <div>
            </div>
                    ...
                    <form class="">
                        <div class="form-row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="form-group row width-100">
                                    <label class="col-12 control-label">{{ trans('lang.otp_verification') }}</label> <!-- Using existing lang key or generic -->
                                    <div class="col-12">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const table = $('#payoutRequestTable').DataTable({
                ...
                order: (driverId == '') ? [4, 'desc'] : [3, 'desc'],
                columnDefs: [{
                        orderable: false,
                        targets: (driverId == '') ? [0, 5, 7] : [0, 4, 6],
                    },
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success == true) {
                            ...
                            window.scrollTo(0, 0);
                            database.collection('driver_payouts').doc(data['payoutId']).update({
        ...
        $(document).on("click", "#submit_cancel", async function(e) {
            ...
            database.collection('users').where("id", "==", auth).get().then(function(resultdriver) {
                if (resultdriver.docs.length) {
                    ...
                    if (!isNaN(price)) {
                        database.collection('driver_payouts').doc(id).update({
                            'paymentStatus': 'Reject',
                            'adminNote': admin_note
                        }).then(function(result) {
                            database.collection('users').doc(driver.id).update({
                                'wallet_amount': price
                            }).then(async function(result) {
        ...
        $(document).on("click", "#submit_accept", async function(e) {
            ...
            database.collection('driver_payouts').doc(id).update(updateData).then(async function(result) {
                if (user && user != undefined) {
