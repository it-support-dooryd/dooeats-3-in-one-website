...
<script type="text/javascript">
    ...
    // Make location strings available to location-selector.js
    window.locationStrings = {
        useCurrentLocation: '{{ trans("lang.use_current_location") }}',
        notSupported: '{{ trans("lang.location_not_supported") }}',
        permissionDenied: '{{ trans("lang.location_permission_denied") }}',
        fetchError: '{{ trans("lang.location_fetch_error") }}',
        noRecent: '{{ trans("lang.no_recent_locations") }}'
    };
    
    // Sync modal search input with hidden field for autocomplete
    function syncLocationInput(value) {
        var hiddenInput = document.getElementById('user_locationnew');
        if (hiddenInput) {
            hiddenInput.value = value;
        }
    }
    
    function syncLocationInputFocus() {
        var hiddenInput = document.getElementById('user_locationnew');
        var modalInput = document.getElementById('locationSearchInput');
        if (hiddenInput && modalInput) {
            // Trigger focus on hidden input to activate autocomplete
